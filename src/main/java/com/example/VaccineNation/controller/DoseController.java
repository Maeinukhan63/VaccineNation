...
public class DoseController {
    ...
    public Dose addDose(@RequestParam("id") int patientId, @RequestParam VaccineBrand vaccineBrand){
           return doseService.addDose(patientId,vaccineBrand);
    }

    @GetMapping("/get")
    public Dose getDose(@RequestParam("id") int Id){
        return doseService.getDose(Id);
    }

    @DeleteMapping("delete/{id}")
    public String DeleteDose(@PathVariable("id") int Id){
        return doseService.DeleteDose(Id);
    }
}
